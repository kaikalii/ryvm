...
use crate::{adjust_i, CloneCell, CloneLock, Control};
...
impl Loop {
    ...
    fn loop_i(&self, state_i: u32, state_tempo: f32) -> Option<u32> {
    ...
    pub fn record(
        ...
        if self.loop_state == LoopState::Recording {
            ...
            let period = period.map(|p| (p as f32 * self.length).round() as u32);
